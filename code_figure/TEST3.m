...
I = linspace(10^-15,5*10^-5,num);               %接收光脉冲功率大小,为变量  为了最后加权计算BER
dI =(max(I)-min(I))./num;                      %步长

prI = @(I) 1./(sqrt(2*pi.*sigma_I_0L_qiang2))./(I).*exp(-(log(I./I_0L) + ...
             sigma_I_0L_qiang2./2).^2./(2.*sigma_I_0L_qiang2));
         
   PIji = integral(prI,10^-15,5*10^-5)
%-------------------------------  下行链路概率密度函数计算   -------------------------------%
%强湍流情况下
...
delta_v=linspace(1,800,150)*10^3;  
...
%%
% AAA =ones(1,1001);
% a = AAA*[pr_I3(1)./2;pr_I3(2:num-1);pr_I3(num)./2].* dI
%%
delta_v1 = delta_v / 1000;
rate_dark =  phase_dark./max(n_rms);
rate_ruo =   phase_ruo./max(n_rms);
rate_qiang1 =  phase_qiang1./max(n_rms);
rate_qiang2 =  phase_qiang2./max(n_rms);

rate_fianl = [rate_dark;rate_ruo;rate_qiang1;rate_qiang2];
%%
%%
SNR111 = signal_power_jieshou./(phase_qiang1+hundun_qiang1);
...
hold on;
box on;
plot(delta_v1,log10(BPSK_BER_dark),'-bd','MarkerIndices',1:10:length(delta_v1),'LineWidth',3);
plot(delta_v1,log10(BPSK_BER_ruo),'-g>','MarkerIndices',1:10:length(delta_v1),'LineWidth',3);
plot(delta_v1,log10(BPSK_BER_qiang1),'-rp','MarkerIndices',1:10:length(delta_v1),'LineWidth',3);
plot(delta_v1,log10(BPSK_BER_qiang2),'-m+','MarkerIndices',1:10:length(delta_v1),'LineWidth',3);
% title("强湍流和弱湍流,夜视情况下的PDF(log)");
